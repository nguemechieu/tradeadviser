...
export const AuthProvider = ({ children }) => {
    const [auth, setAuth] = useState(() => {
        ...
        try {
            ...
            if (savedToken && savedUser) {
                ...
                const role = user.role || user.user_role || 'trader';
                console.debug('Auth initialized from storage:', { email: user.email, role });
                return {
                    token: savedToken,
                    user: user,
                    role: role
    ...
    useEffect(() => {
        if (auth?.token && auth?.user) {
            ...
            console.debug('Auth persisted to storage:', { email: auth.user.email, role: auth.role });
        } else {
