...
class ServerServiceContainer:
    ...
    user_repository: UserRepository | None = field(default=None, init=False)
    ...
    async def refresh_access(self, refresh_token: str) -> dict[str, Any]:
        ...
        record = self.refresh_tokens.get(normalized)

        if record is None or record.refresh_expires_at <= _utcnow():
            self.refresh_tokens.pop(normalized, None)
        ...
        return await self.authenticate_access(
            user.email,
            user.password,
            remember_me=True,
        )
    ...
    async def logout_user(
        ...
        if refresh_token:
            ...
            record = self.refresh_tokens.pop(normalized, None)
            if record is not None:
                self.tokens.pop(record.token, None)
        else:
            for token, record in list(self.tokens.items()):
    ...
    async def get_user_profile(self, user_id: str) -> dict[str, Any]:
        user = self.users_by_id.get(str(user_id or "").strip())
    ...
    async def update_user_profile(
        ...
        if "username" in clean_data and clean_data["username"]:
            new_username = _normalize_username(str(clean_data["username"]))

            for existing in self.users_by_id.values():
                if existing.user_id != user.user_id and existing.username == new_username:
                    raise ValueError("Username is already taken.")

            user.username = new_username

        if "display_name" in clean_data and clean_data["display_name"]:
            user.display_name = str(clean_data["display_name"]).strip()
    ...
    async def is_username_available(self, username: str) -> bool:
        ...
        return all(user.username != normalized for user in self.users_by_id.values())
    ...
    async def is_email_available(self, email: str) -> bool:
        ...
        return normalized not in self.users
    ...
    async def deactivate_account(
        ...
        await self.logout_all_sessions(user.user_id)
    ...
    async def setup_two_factor(
        ...
        return {
            ...
            "user_id": user.user_id,
            "secret": secret,
            "provisioning_uri": f"otpauth://totp/TradeAdviser:{user.email}?secret={secret}&issuer=TradeAdviser",
        }
    ...
    async def verify_two_factor(
        ...
        if not str(code or "").strip():
            raise ValueError("Two-factor code is required.")
    ...
    async def create_api_key(
        ...
        clean_scopes = sorted(set(str(scope).strip() for scope in scopes if str(scope).strip()))

        invalid_scopes = [scope for scope in clean_scopes if scope not in TRADEADVISER_API_KEY_SCOPES]
        if invalid_scopes:
            raise ValueError(f"Invalid API key scopes: {', '.join(invalid_scopes)}")

        raw_key = f"ta_live_{token_hex(32)}"
        key_id = f"key_{token_hex(8)}"
    ...
    async def get_api_key(
        ...
        record = self.api_keys.get(str(key_id or "").strip())
    ...
    async def update_api_key(
        ...
        if "name" in data and data["name"]:
            record.name = str(data["name"]).strip()

        if "scopes" in data and data["scopes"] is not None:
            clean_scopes = sorted(
                set(str(scope).strip() for scope in data["scopes"] if str(scope).strip())
            )

            invalid_scopes = [
                scope for scope in clean_scopes
    ...
    # ============================================================
    # Portfolio / Trading Demo Runtime
    # ============================================================
    ...
    async def place_order(
        ...
        side = str(
            execution_request.get("side")
            or execution_request.get("order_side")
            or "buy"
        ).strip().lower()

        order_type = str(
            execution_request.get("type")
            or execution_request.get("order_type")
            or "market"
        ).strip().lower()
        ...
        session_id = str(
            execution_request.get("session_id")
            or command.get("session_id")
            or ""
        ).strip()
        ...
        if session_id:
            await self.send_event(
                ...
                "order_updated",
                {
                    ...
                    "client_order_id": str(
                        execution_request.get("client_order_id") or order["order_id"]
                    ),
                    "status": order["status"],
                    "symbol": symbol,
                    "side": side,
    ...
    def resolve_session_user(self, session_id: str) -> User | None:
        session = self.sessions.get(str(session_id or "").strip())
    ...
    def performance(self, user: User) -> dict[str, Any]:
    ...
    def get_workspace_settings(self, user: User) -> dict[str, Any]:
    ...
    def get_portfolio_dashboard(self, user: User) -> dict[str, Any]:
    ...
    def list_positions(self, user: User) -> list[dict[str, Any]]:
        positions = []
    ...
    def list_orders(
        ...
        rows = [
            dict(order)
            for order in list(self.orders_by_user.get(user.user_id, []) or [])
        ]
        ...
        if active_only:
            rows = [
                ...
                if str(row.get("status") or "").lower()
                   in {"open", "submitted", "accepted", "new"}
            ]
    ...
    def admin_overview(self, admin_user: User) -> dict[str, Any]:
        self._require_admin(admin_user)

        open_positions = sum(
            len(self.list_positions(user))
            for user in self.users_by_id.values()
        )

        open_orders = sum(
            len(self.list_orders(user, active_only=True))
            for user in self.users_by_id.values()
        )
        ...
        return {
            ...
            "recent_users": [
                user.as_public_dict()
                for user in list(self.users_by_id.values())[:8]
            ],
        }

    def admin_list_users(self, admin_user: User) -> list[dict[str, Any]]:
        self._require_admin(admin_user)
    ...
    def _seed_demo_state(self) -> None:
        ...
        if operator is not None:
            ...
            self.record_signal(
                operator,
                {
                    ...
                    "confidence": 0.74,
                    "timeframe": "4h",
                    "message": "Gold breakout setup forming.",
                },
            )
        ...
        if trader is not None:
            ...
            self.record_signal(
                trader,
                {
                    ...
                    "confidence": 0.81,
                    "timeframe": "1d",
                    "message": "Watch for continuation above 890.",
                },
            )
        ...
        if admin is not None:
            self.record_signal(
                ...
                {
                    "symbol": "DESK",
                    "strategy": "Admin Pulse",
                    "confidence": 1.0,
                    "timeframe": "live",
                    "message": "Admin dashboard seeded successfully.",
                },
            )

    def _seed_workspace(self, user:User) -> None:
    ...
    def _resolve_actor(
        ...
        session_id = str(
            ...
            or (payload.get("execution_request") or {}).get("session_id")
            or ""
        ).strip()
    ...
    def _portfolio_summary(self, user: User) -> dict[str, Any]:
        positions = self.list_positions(user)
    ...
    def _apply_fill(
        ...
        if current_qty != 0.0 and (
                current_qty > 0 > signed_trade_qty
                or current_qty < 0 < signed_trade_qty
        ):
        ...
        if target is None and new_qty != 0.0:
            ...
            if new_qty == 0.0:
                ...
                if (
                        current_qty == 0
                        or (current_qty > 0 and signed_trade_qty > 0)
                        or (current_qty < 0 and signed_trade_qty < 0)
                ):
                    weighted_entry = (
                        ((entry_price * old_abs) + (price * abs(signed_trade_qty))) / new_abs
                        if new_abs
                        else price
                    )
    ...
    async def resend_email_verification(self, param):
        pass

    async def list_user_sessions(self, user_id):
        pass

    async def revoke_user_session(self, user_id, session_id):
        pass
...
async def get_services(
    ...
    For now, this keeps the in-memory demo state global.
    The db session is available for repositories when you migrate
    methods from memory to the real database.
    """

    if _services.db is None:
        _services.db = db
        _services.user_repository = UserRepository(db)
