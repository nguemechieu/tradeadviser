...
def _coerce_config(config, *, exchange: str, default_subtype: str = "swap"):
...
class BinanceFuturesBroker(CCXTBroker):
    def __init__(self, config) -> None:
        normalized = _coerce_config(config, exchange=str(getattr(config, "exchange", None) or "binance").lower())
